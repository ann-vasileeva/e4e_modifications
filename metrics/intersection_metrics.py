...
def metrics_torch(y_true, y_pred, metric_name,
    ...
    if not flag_soft:
        if num_classes > 1:
            y_pred = torch.stack([(torch.argmax(y_pred, dim=1)==i) for i in range(num_classes)]).permute(1,2,3,0).float()
            y_true = torch.stack([(torch.argmax(y_true, dim=1)==i) for i in range(num_classes)]).permute(1,2,3,0).float()
